 ...
 * Sends tokens at the end
 * 
 * Lifecycle: deploy, add participants, set params, lock, take participance until end by time or hardcap,
 * ..check results
 */
...
contract Ico {
	...
	function setParams(
		...
		uint _tokenCount,
		uint _startTimeInMinutes,
		uint _endTimeInMinutes,
		uint _discount,
		uint _minPurchase,
		...
		startTime=_startTimeInMinutes * 1 minutes + now; 
		endTime=_endTimeInMinutes * 1 minutes + now;
		discount=_discount; 
	...
	function participance (address sender, uint value) private {
		uint amount = value;
